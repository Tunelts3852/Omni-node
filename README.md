...
노드는 카드 전체를 바로 드래그해 배치할 수 있고, 배경 드래그로 팬, 휠로 줌, 출력 포트 점에서 입력 포트 점으로 드래그해서 연결한다. 리사이즈 핸들로 크기를 조절할 수 있고, 노드별 최소 크기가 적용돼 내용이 깨지지 않는다. 인스펙터에서는 `직접 입력 / 목록에서 고르기 / 연결해서 받기` 방식으로 값을 넣고, 파일/메모리 경로는 `찾아보기`로 고를 수 있다. 저장된 그래프는 기존 루틴 저장소 안에 `logic_graph` 실행 모드로 보관되지만, 일반 루틴 탭 목록에는 섞이지 않는다.
