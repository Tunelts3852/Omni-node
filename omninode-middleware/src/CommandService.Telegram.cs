    ...
    )
    {
        ...
        if (webSearchEnabled && snapshot.Mode == "single")
        {
            ...
            if (webDecision.NeedWeb || shouldFallbackToGeminiWeb)
            {
                ...
                var webSingle = await GenerateGeminiGroundedWebAnswerAsync(
                    text,
                    memoryHint,
                    selfDecideNeedWeb: shouldFallbackToGeminiWeb,
                    allowMarkdownTable: allowMarkdownTable,
                    enforceTelegramOutputStyle: true,
                    cancellationToken
                );
                var webResponseText = $"[Single {webSingle.Provider}:{webSingle.Model}]\n{FormatTelegramResponse(webSingle.Text, TelegramMaxResponseChars)}";
                var webAssistantMeta = $"telegram-single:{webSingle.Provider}:{webSingle.Model}:gemini-web-single";
                _conversationStore.AppendMessage(session.Thread.Id, "user", text, "telegram:user");
                _conversationStore.AppendMessage(session.Thread.Id, "assistant", webResponseText, webAssistantMeta);
                await EnsureConversationTitleFromFirstTurnAsync(session.Thread.Id, webSingle.Provider, webSingle.Model, cancellationToken);
                _ = await MaybeCompressConversationAsync(session.Thread.Id, "chat-single", webSingle.Provider, webSingle.Model, cancellationToken);
